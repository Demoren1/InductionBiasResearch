...
def main():
    ...
    radius = protocol["search"]["latent_radius"]
    ...
    for stage in ("initial", "final"):
        ...
        diagnostics["optimization"][stage] = {
            ...
            "latents_at_radius_bound": int((norm >= radius - 1e-5).sum())}
    ...
    axes[0].semilogy(history.mean(axis=1), label=f"Gradient search: mean over {n_starts} starts")
    ...
    lines = ["# Два замороженных VAE: поиск согласованных масок", "",
             f"Два VAE seeds {model_seeds[0]}/{model_seeds[1]}, один OOD split, "
             f"{n_starts} независимых latent-стартов. "
    ...
    for name, softloss in (("initial", opt["initial_loss"]), ("optimized", opt["final_loss"]), ("random_search", random["loss"])):
        ...
        lines.append(f"| {name} | {float(softloss.mean()):.6f} | {record['iou']['mean']:.4f} | {record['hamming']['mean']:.2f} | {record['exact_matches']}/{n_starts} |")
    lines += ["", "## Независимая проверка структуры и качества", "",
              f"Accuracy — среднее по четырём held-out задачам, {n_starts} масок на метод; свежие MLP, "
    ...
    lines += ["", f"Среднее изменение Gold IoU после оптимизации: **{delta['optimized_minus_initial_iou']['mean']:+.4f}**; "
              f"рост у {delta['optimized_minus_initial_iou']['starts_improved']}/{n_starts} пар стартов.", "",
    ...
    lines.append("Число разных масок с точностью до перестановки колонок: " + "; ".join(
        f"{name}: {count}/{n_starts}" for name, count in diagnostics["unique_masks_modulo_columns"].items()) + ".")
    lines.append("")
    ...
    for stage in ("initial", "final"):
        ...
        lines.append(f"- {stage}: средняя норма latent {d['latent_norm_mean']:.3f}, "
                     f"на границе радиуса {radius:g} — {d['latents_at_radius_bound']}/{2 * n_starts}; "
                     f"среднее S(1−S) = {d['mean_softness_s_times_one_minus_s']:.4f} (0 для бинарных значений, 0.25 для 0.5).")
    proposals = protocol["search"]["steps"] + 1
    lines += ["", f"Это одна pair seeds и одно разбиение задач. {n_starts} latent-стартов не заменяют повторение с другими VAE. "
              f"Случайный поиск сопоставлен по числу независимых пар-кандидатов ({proposals} на старт), а не по времени вычислений. "
