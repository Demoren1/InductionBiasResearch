...
from models.cvae import (  # noqa: E402
    CVAE, cvae_loss, make_importance_loss,
...
def cpu_state_dict(model: torch.nn.Module) -> dict:
    """Return a device-independent model checkpoint."""
...
def plot_samples(samples: dict, out_dir: Path) -> Path:
    """Plot a few generated masks for each pattern."""
    n_rows = len(samples)
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_log[-1] < best_val:
                best_val = val_log[-1]
                torch.save(cpu_state_dict(model), out_dir / "cvae_best.pt")
        ...
        meta = {
            ...
            "importance_maps": args.importance_maps,
            "importance_name": args.importance_name,
            "seed": args.seed,
...
def do_sample(args) -> None:
    ...
    for pat in args.patterns:
        ...
        if args.k_active:
            m = model.sample_topk(lab, n_per, args.k_active, generator=gen).cpu()
        else:
            m = model.sample(lab, n_per, generator=gen).cpu()
        out[pat] = m.reshape(n_per, config.SEQ_LEN, config.H)
