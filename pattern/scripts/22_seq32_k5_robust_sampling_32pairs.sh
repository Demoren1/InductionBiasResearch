#!/usr/bin/env bash
# Run R=4 robust hard-mask sampling for all 32 seq32/k5 VAE pairs.
set -euo pipefail
...
PARENT="${PARENT:-pattern/outputs/fixed_k5_agreement/sampling_32pairs_20260913}"
OUT_DIR="${OUT_DIR:-$PARENT/robust_sampling_r4}"
GPU_IDS="${GPU_IDS:-0 1 2 3 4 5 6 7}"
read -r -a GPUS <<< "$GPU_IDS"

python -m pattern.fixed_k5_agreement.robust_sampling \
  --stage prepare --parent "$PARENT" --out "$OUT_DIR"
...
python -m pattern.fixed_k5_agreement.robust_sampling --stage report --out "$OUT_DIR"
