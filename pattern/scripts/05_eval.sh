...
EVAL_EXACT32="${EVAL_EXACT32:-0}"
set -euo pipefail
cd "$(dirname "$0")/.."
NUM_GPUS=$(echo "$GPU_IDS" | wc -w)
echo "eval on ${NUM_GPUS} GPU(s): ${GPU_IDS}"
EXTRA_ARGS=()
if [ "$EVAL_EXACT32" = "1" ]; then
    EXTRA_ARGS+=(--include_exact32)
fi
PIDS=()
gpu_idx=0
for gpu in $GPU_IDS; do
    CUDA_VISIBLE_DEVICES="$gpu" python evaluation/eval_generated_masks.py \
        ...
        --out_suffix "$EVAL_SUFFIX" "${EXTRA_ARGS[@]}" &
    PIDS+=($!)
    gpu_idx=$((gpu_idx + 1))
done
for pid in "${PIDS[@]}"; do
...
python evaluation/merge_eval.py --num_gpus "$NUM_GPUS" --out_suffix "$EVAL_SUFFIX"
