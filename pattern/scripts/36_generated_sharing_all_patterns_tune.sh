...
OUT_ROOT="${OUT_ROOT:-pattern/outputs/bilevel_mask/generated_sharing_all_patterns_tuning_20260915}"
...
for inner_lr in 0.03 0.1; do
  for inner_steps in 10 20 50; do
    for generator_lr in 0.001 0.003 0.01; do
      ...
      CUDA_VISIBLE_DEVICES="$gpu" python -m pattern.bilevel_mask.tune_generated_sharing \
        --output "$OUT_ROOT/$tag" --device cuda --seed 42 --pattern-coverage all16 \
        --inner-lr "$inner_lr" --inner-steps "$inner_steps" \
        --generator-lr "$generator_lr" --outer-steps 100 \
        --train-restarts 8 --eval-restarts 32 --final-refit-steps 1000 \
