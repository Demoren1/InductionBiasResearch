...
class FirstLayerU(nn.Module):
    def __init__(self, arm: str, seed: int) -> None:
        ...
        initial_u = (convolution_u() if arm == "convolution"
                     else torch.randn(N_WEIGHTS, RANK))
...
def evaluate(model: FirstLayerU, pool: tuple, *, seed: int, tasks: int,
    ...
    for length in lengths:
        ...
        for score in scores:
            ...
            for _ in range(steps):
                prediction = model.predict(s[0], s[1], v1, readout, u)
            ...
            prediction = model.predict(q[0], q[1], v1, readout, u)
...
def convolution_overlap(model: FirstLayerU) -> float | None:
    if model.arm == "dense":
        return None
    reference = convolution_u().to(model.u_raw.device)
