"""Run all eight independent combined-search shards on GPU 0 through 7."""
import argparse
import os
...
def main():
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "outputs/combined_task_agreement/20260906")
    ...
    try:
        for gpu, (profile, gap, shard) in enumerate(jobs):
            ...
            command = [sys.executable, str(ROOT / "evaluation/combined_task_agreement.py"),
                       "--out", str(args.out), "--profile", profile, "--gap", str(gap), "--shard", str(shard)]
            process = subprocess.Popen(command, stdout=stream, stderr=subprocess.STDOUT, env=env, cwd=ROOT)
            running[label] = (process, stream)
        ...
        while running:
            for label, (process, stream) in list(running.items()):
                ...
                if result is not None:
                    ...
                    stream.close(); del running[label]
                    print(f"EXIT {label} code={result}", flush=True)
            if time.monotonic() - last_status >= 30:
                print(f"STATUS running={len(running)} complete={len(completed)}", flush=True)
                last_status = time.monotonic()
            if running:
                time.sleep(2)
        ...
        for process, _ in running.values():
            process.terminate()
        for process, stream in running.values():
            process.wait(); stream.close()
        raise
    if any(completed.values()):
        raise SystemExit(f"Some workers failed: {completed}")
