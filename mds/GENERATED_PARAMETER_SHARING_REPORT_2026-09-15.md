...
В текущем эксперименте один фиксированный `z*` порождает одну общую структуру. Поэтому результат подтверждает generated-параметризацию `U`. [Проверка роли латента](FINAL_REPORT_2026-09-19.md) показала, что обучение `z` улучшает качество относительно одного фиксированного кода, но 16 разных фиксированных кодов достигают сопоставимого качества при близком времени обучения. Перенос на новые классы задач и польза разных `z` для разных задач здесь не проверены.
