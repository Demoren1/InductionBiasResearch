...
Матрица `U` была разной по смыслу. В generated parameter sharing и опытах Yeh
её строки задают категорию общего параметра и после обучения становятся
one-hot. В meta-pattern, исходном MNIST8m и Aug-Omniglot `U` была непрерывным
базисом. Дополнительный MNIST8m-контроль ниже отдельно проверяет бинарную
assignment-матрицу.

Основной результат: непрерывная сгенерированная `U` часто лучше случайной или
фиксированной структуры. Она может превзойти кронекеровскую параметризацию и
прямую оптимизацию assignment-матрицы, но **не превосходит прямо обучаемую `U`
во всех задачах**. На Aug-Omniglot прямая `U` лучше генератора; на MNIST8m
непрерывный генератор почти равен плотной MLP на официальном test-50 и
устойчивее к смене блока изображений. Бинарная generated `U` на MNIST8m не
улучшила случайную бинарную. Польза градиентного обучения латента `z` не
подтверждена.
...
| MNIST8m, бинарная assignment U | Координаты веса; мягкая U при обучении, one-hot при тесте | **79,00%** против **78,96%** у случайной бинарной U | Генерация не улучшила бинарный контроль; continuous generated U лучше на 4,66 п.п. |
...
| Generated binary U | 79,00% | 0,548 |
| Фиксированная random binary U | 78,96% | 0,558 |
...
Отдельно проверили бинарную `U` по протоколу Yeh: при meta-обучении каждая
строка была непрерывным распределением по 32 значениям `v`, а на валидации,
обычном DigitSum и тесте заменялась на one-hot `argmax`. Из шести настроек
температуры и регуляризации выбрали лучшую только по hard-валидации. После
заморозки `U` generated binary получила **79,00%** и MAE **0,548** на длине 50,
случайная бинарная — **78,96%** и **0,558**. Разница **0,04 п.п.** не показывает
пользы генератора; непрерывная generated `U` лучше бинарной на **4,66 п.п.**
...
[бинарная U и подбор](../deepsets_z/mnist8m/binary_u_summary.json) ·
