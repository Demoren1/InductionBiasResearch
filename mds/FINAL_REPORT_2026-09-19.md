...
Чтобы выделить вклад генератора, при одном seed 42 и одном свёрточном роутере сравнили четыре `U` размера `235500 × 32`: сгенерированную, фиксированную случайную, аналитическую для 3×3-свёртки и обучаемую кронекеровскую. Последняя задаёт первый слой как `W = A V Bᵀ`, где размеры `A`, `V` и `B` равны `785 × 8`, `8 × 4` и `300 × 4`. У генератора 7 840 параметров, у двух кронекеровских множителей — 7 480; во всех вариантах `v` содержит 32 числа.
...
**Вывод.** При согласованном свёрточном роутере сгенерированная `U` на длине 50 лучше аналитической свёрточной на **3,07 п.п.**, кронекеровской на **7,23 п.п.** и случайной на **11,12 п.п.** Кронекеровская параметризация близка к генератору по числу параметров, поэтому разрыв не объясняется размером модели. Генератор также лучше всех трёх rank-32 контролей по MAE. Однако он лишь сравнялся с плотной сетью по точности (**83,66% против 83,72%**) и уступил ей по MAE (**0,441 против 0,391**).
