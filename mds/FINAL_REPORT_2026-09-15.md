# Final report: Pattern experiments and generated parameter sharing
...
Проверена замена прямой оптимизации assignment-матрицы `A` из Yeh et al. на генератор структуры `A=Gψ(z)`. В Gaussian один генератор обучался на всех оцениваемых задачах; в cross-correlation и denoising один генератор разделялся между пятью задачами. Для последних двух постановок сравнивались общий и task-specific латент `z`.

### Test loss

| Benchmark | Paper direct `A` | Our direct `A` | Generated `A` | No sharing | Oracle |
|---|---:|---:|---:|---:|---:|
| Gaussian `K=2` | 0.014 ± 0.003 | 0.016421 ± 0.0027 | **0.010841 ± 0.0019** | 0.022298 ± 0.0028 | 0.010841 ± 0.0019 |
| Gaussian `K=4` | 0.025 ± 0.003 | 0.023992 ± 0.004 | **0.0094668 ± 0.0020** | 0.039626 ± 0.0040 | 0.0094668 ± 0.0020 |
| Gaussian `K=6` | 0.028 ± 0.004 | 0.030741 ± 0.005 | **0.0084429 ± 0.0017** | 0.058994 ± 0.0049 | 0.0084429 ± 0.0017 |
| Denoising `K=8`, global `z` | — | — | **8.9169 ± 0.044** | 9.1391 ± 0.26 | 8.8388 ± 0.062 |
| Denoising `K=8`, task-specific `z` | — | — | **8.6676 ± 0.094** | 9.1391 ± 0.26 | 8.8388 ± 0.062 |
| Cross-correlation `A=6`, global `z` | — | — | **0.00015609 ± 3.9e−5** | 0.00019193 ± 2.8e−5 | 0.00010896 ± 6.5e−5 |
| Cross-correlation `A=6`, task-specific `z` | — | — | **0.00015609 ± 3.9e−5** | 0.00019193 ± 2.8e−5 | 0.00010896 ± 6.5e−5 |
| Cross-correlation `A=15`, global `z` | — | — | 0.40269 ± 0.0059 | **0.00029336 ± 7.4e−5** | 0.000073399 ± 4.1e−5 |
| Cross-correlation `A=15`, task-specific `z` | — | — | **0.00018476 ± 3.5e−5** | 0.00029336 ± 7.4e−5 | 0.000073399 ± 4.1e−5 |

### Partition distance

| Benchmark | Paper direct `A` | Our direct `A` | Generated `A` | No sharing | Oracle |
|---|---:|---:|---:|---:|---:|
| Gaussian `K=2` | 0.145 ± 0.050 | 0.185 ± 0.054 | **0** | 1 | 0 |
| Gaussian `K=4` | 0.490 ± 0.095 | 0.435 ± 0.092 | **0** | 3 | 0 |
| Gaussian `K=6` | 0.590 ± 0.130 | 0.675 ± 0.140 | **0** | 5 | 0 |
| Denoising `K=8`, global `z` | — | — | 32.0 | 49 | **0** |
| Denoising `K=8`, task-specific `z` | — | — | 31.6 ± 1.2 | 49 | **0** |
| Cross-correlation `A=6`, global/task-specific `z` | **0** | — | 1 | 3 | **0** |
...
| Проверка | Результат |
|---|---|
| Gaussian | Generated `A` достигает oracle loss и нулевого partition distance; лучше опубликованной и нашей прямой оптимизации `A` |
| Cross-correlation `A=6` | Generated `A` лучше no-sharing по test loss примерно на 19% при обоих вариантах `z` |
| Cross-correlation `A=15` | Task-specific `z` лучше no-sharing примерно на 37%; общий `z` не сработал |
| Denoising | Global `z` лучше no-sharing примерно на 2%, task-specific `z` — примерно на 5% |
| Восстановление аналитической структуры | Подтверждено на Gaussian; не подтверждено на cross-correlation и denoising, несмотря на улучшение predictive loss |

Полученные результаты поддерживают основной тезис: генератор способен задавать полезное компактное семейство структур parameter sharing и по predictive loss превосходить прямое обучение без sharing. Они пока не доказывают перенос inductive bias на новые задачи: Gaussian-эксперимент transductive, а в остальных строках выбран лучший по validation из четырёх рестартов. Официальный release не содержит `ConvSharing`, поэтому cross-correlation и denoising воспроизведены по постановке статьи, но не являются побитовой репликацией авторского кода.

Источники: [статья Yeh et al.](https://proceedings.mlr.press/v151/yeh22b/yeh22b.pdf), [официальный код](https://github.com/raymondyeh07/equivariance_discovery), [полный standalone-отчёт](../yeh2022_generated_sharing/FINAL_REPORT.md).
